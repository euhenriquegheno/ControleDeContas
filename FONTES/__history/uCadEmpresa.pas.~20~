...
begin
  Try
    If Application.MessageBox('Deseja realmente excluir a Empresa?', 'ATEN플O', mb_iconquestion + mb_yesno) = idYes then
    qrEmpresa.delete;
  Except
...
begin
  ...
  Else
  Begin
    Application.MessageBox('Voce so pode acessar essa aba inserindo ou editando alguma Empresa!', 'ATEN플O', MB_ICONEXCLAMATION);
    pcPrincipal.ActivePage := tsConsulta;
  End;
end;
