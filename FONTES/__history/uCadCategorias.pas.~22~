...
begin
  Try
    If Application.MessageBox('Deseja realmente excluir a Catetgoria?', 'ATEN플O', mb_iconquestion + mb_yesno) = idYes then
      qrCategorias.delete;
  Except
...
begin
  ...
  Else
  Begin
    Application.MessageBox('Voce so pode acessar essa aba inserindo ou editando alguma categoria!', 'ATEN플O', MB_ICONEXCLAMATION);
    pcPrincipal.ActivePage := tsConsulta;
  End;
end;
