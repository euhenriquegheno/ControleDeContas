...
begin
  try
    ...
    if qrdata.RecordCount > 0 then // já existe
    begin
      if messagedlg('Já existe um caixa aberto com esta data. É aconselhável não realizar nova abertura.' + #13 + 'Deseja abrir mesmo assim?', mtConfirmation, [mbYes, mbNo], 0) = mrNo then
      begin
        Exit;
        Close;
      end;
...
//        FieldByName('codigo').asstring := frmprincipal.getCodigo('c000044', 10);
    ...
    on E: Exception do
    begin
      showmessage('Esta função não pode ser processada corretamente, a execução da mesma foi cancelada!' + sLineBreak + E.Message);
      dm.Conexao.Rollback;
    end;
  end;
